...
- M2e 已实现:模型目录(38 个生成 shard、500+ 模型,嵌入二进制)、`Models` 集合(getAuth/stream 路由、refresh/store、动态 provider)、40 个内置 provider 工厂注册表、faux provider、图像 API。config.toml 新增可选 `cost` 定价表(上游 `ModelCost` 形状:camelCase `cacheRead`/`cacheWrite` + 可选 `tiers`)。packages/ai 移植完成,仅余 M2f 加固清单;Models 接入 CLI 默认路径随 M3 落地,当前 CLI 保持直连 provider 流程。
